...
MSSFMLController::MSSFMLController(sf::RenderWindow &window, MinesweeperBoard &board, MSSFMLView &view) : mswindow(window), msboard(board),  msview(view) {}

void MSSFMLController::EventLoop() {
    sf::Event event;
    fieldSize = msview.getFieldSize();
    fieldBorder = msview.getSpaceBetween();
    while (mswindow.pollEvent(event)) {
